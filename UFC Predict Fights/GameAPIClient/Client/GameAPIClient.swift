...
final class GameAPIClient {
    ...
    private func request(_ endpoint: String, method: String = "GET", body: Encodable? = nil) async throws -> Data {
        ...
        // Breadcrumb
        CrashReporter.log("API \(method) \(endpoint)")
        ...
        case 200..<300:
            return data
        case 401:
            CrashReporter.recordAPIError(endpoint: endpoint, statusCode: 401, message: "Token expired")
            throw APIError.unauthorized
        case 403:
            let detail = parseDetail(data)
            CrashReporter.recordAPIError(endpoint: endpoint, statusCode: 403, message: detail)
        ...
        case 422:
            ...
            throw APIError.validationError(detail)
        case 429:
            CrashReporter.recordAPIError(endpoint: endpoint, statusCode: 429, message: "Rate limited")
            throw APIError.serverError(429)
        default:
            CrashReporter.recordAPIError(endpoint: endpoint, statusCode: http.statusCode, message: "Server error")
    ...
    // MARK: - Protected Endpoints (migrated from BlackSpartan)
    
    func getFighterProfile(id: Int) async throws -> BSFighterProfile {
        let data = try await request("/fighters/\(id)/profile")
        return try JSONDecoder.bsDecoder.decode(BSFighterProfile.self, from: data)
    }
    
    func getEventDetail(id: Int) async throws -> BSEventDetail {
        let data = try await request("/events/\(id)")
    ...
    func predictFight(fighterAId: Int, fighterBId: Int) async throws -> Prediction {
        let body = PredictRequest(fighterAId: fighterAId, fighterBId: fighterBId)
        let data = try await request("/predict/", method: "POST", body: body)
        return try JSONDecoder.bsDecoder.decode(Prediction.self, from: data)
    }
}
